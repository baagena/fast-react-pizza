...
function Button({ children, disabled, to, type, onclick }) {
  ...
  const styles = {
    ...
    small: base + " px-4 py-2 text-xs md:px-5 md:py-3",
    round: base + " px-3 py-1.5 text-sm md:px-3.5 md:py-2",
    secondary:
      "inline-block text-sm rounded-full bg-transparent border-2 border-stone-300  font-semibold uppercase tracking-wide text-stone-800 transition-colors duration-300 hover:text-stone-800 hover:bg-stone-300  focus:bg-stone-300 focus:text-stone-800  focus:outline-none focus:ring focus:ring-stone-300 focus:ring-offset-2 disabled:cursor-not-allowed px-4 py-2.5 md:px-6 md:py-3.5",
  };
  if (to) {
    return (
